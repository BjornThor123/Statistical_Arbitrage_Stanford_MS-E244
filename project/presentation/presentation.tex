...
\begin{frame}{Step 1 --- Constructing the Skew}
  \textbf{For each day and each ticker, fix a target $\tau^*$ and $\Delta^* = 0.25$:}
  \vspace{0.4em}
  \begin{enumerate}
    \item Use \textbf{OTM options only} (higher liquidity; avoids early-exercise noise)
    \item Select the expiry nearest to $\tau^*$
    \item Find the OTM put and OTM call whose $|\Delta|$ is closest to $\Delta^*$
    \item Define the skew as the \textbf{25-delta risk-reversal IV spread}:
    \[
      \text{skew}_t \;=\; \sigma^{\text{put}}_{25\Delta,\,t} \;-\; \sigma^{\text{call}}_{25\Delta,\,t}
    \]
  \end{enumerate}
  \vspace{0.2em}
  \begin{block}{Why this measure?}
    The skew is \emph{positive when puts are expensive} and measures exactly the
    options we trade in the risk reversal --- signal and instrument are aligned
    by construction.
  \end{block}
  \vspace{0.2em}
  \begin{alertblock}{Alternative: polynomial fit}
    Fit $\sigma_{IV}(\kappa) \approx \beta_0 + \beta_1\kappa + \beta_2\kappa^2$
    to all OTM options and use $-\hat\beta_1$ as the skew proxy
    (negated so that high $= $ puts expensive).
    Smoother but less directly tied to the 25$\Delta$ strikes actually traded.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Cumulative Returns}
  \begin{center}
    % Replace with: \includegraphics[width=0.85\textwidth]{../plots/option_skew/cumulative_returns.png}
    \vspace{2cm}
...
\begin{frame}{Annual Returns}
  \begin{center}
    % Replace with: \includegraphics[width=0.85\textwidth]{../plots/option_skew/annual_returns.png}
    \vspace{2cm}
...
\begin{frame}{Drawdown}
  \begin{center}
    % Replace with: \includegraphics[width=0.85\textwidth]{../plots/option_skew/drawdown.png}
    \vspace{2cm}
...
\begin{frame}{P\&L Decomposition}
  \begin{center}
    % Replace with: \includegraphics[width=0.85\textwidth]{../plots/option_skew/pnl_decomposition.png}
    \vspace{2cm}
...
\begin{frame}{Signals}
  \begin{center}
    % Replace with: \includegraphics[width=0.85\textwidth]{../plots/option_skew/signals.png}
    \vspace{2cm}
...
\begin{frame}{Monthly Returns Heatmap}
  \begin{center}
    % Replace with: \includegraphics[width=0.85\textwidth]{../plots/option_skew/monthly_returns.png}
    \vspace{2cm}
...
\begin{frame}{Conclusion \& Future Work}
  \begin{columns}
    \column{0.5\textwidth}
      \textbf{Summary}
      \begin{itemize}
        \item Extracted skew as the 25$\Delta$ put--call IV spread
    ...
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item SVI / signature-method skew parametrisation (beyond polynomial fit)
