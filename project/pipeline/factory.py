...
from signal_models.modules import MomentumSignalGenerator, ResidualZScoreSignalGenerator, RiskReversalSignalGenerator
...
Use build_default_surface_pipeline() for SSVI surface + risk-reversal signals.
Use build_default_smile_pipeline() for simple linear smile + risk-reversal signals.

Example
-------
pipeline = build_default_surface_pipeline(db_path="data/market_data.duckdb")
output = pipeline.run(spec)
"""
...
def build_default_surface_pipeline(
    ...
    if signal_kind == "risk_reversal":
        ...
        signal_generator = MomentumSignalGenerator(**(signal_kwargs or {}))
    else:
        raise ValueError("signal_kind must be one of: risk_reversal, residual, momentum")
    return StrategyPipeline(
        data_loader=DuckDBOptionsLoader(db_path=db_path, table=table),
        data_processor=BasicOptionsProcessor(**(processor_kwargs or {})),
