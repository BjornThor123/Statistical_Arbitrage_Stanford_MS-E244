...
@dataclass
class SimpleCostBacktestEngine(BacktestEngineModule):
    unit_cost: float = 0.02

    def _backtest_one(self, signal_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(df)):
            pos = float(df.loc[i, "position"])
            turnover[i] = abs(pos - prev_pos)
            costs[i] = turnover[i] * self.unit_cost
            if i + 1 < len(df):
                e0 = float(df.loc[i, "residual"]) if np.isfinite(df.loc[i, "residual"]) else np.nan
                e1 = float(df.loc[i + 1, "residual"]) if np.isfinite(df.loc[i + 1, "residual"]) else np.nan
                if np.isfinite(e0) and np.isfinite(e1):
                    gross[i + 1] = pos * (-(e1 - e0))
            pnl[i] = gross[i] - costs[i]
            prev_pos = pos
        ...
        df["cost"] = costs
        df["net_pnl"] = pnl
        df["cum_net_pnl"] = np.cumsum(pnl)
        return df

    def run(self, signals: SignalOutput, skew: SkewOutput, spec: RunSpec) -> BacktestOutput:
        by_ticker = {t: self._backtest_one(df) for t, df in signals.signal_map.items()}
        parts = [
            df[["date", "net_pnl", "gross_pnl", "cost"]].assign(ticker=t)
            for t, df in by_ticker.items()
            if not df.empty
        ]
        if parts:
            all_rows = pd.concat(parts, ignore_index=True)
            portfolio = all_rows.groupby("date", as_index=False).agg(
            ...
            portfolio = pd.DataFrame(columns=["date", "portfolio_net_pnl", "portfolio_gross_pnl", "portfolio_cost", "n_names", "portfolio_cum_net_pnl"])
...
class DeltaHedgedOptionBacktestEngine(BacktestEngineModule):
    ...
    def _backtest_one(self, ticker: str, signal_df: pd.DataFrame, pair_df: pd.DataFrame) -> pd.DataFrame:
        ...
        def _option_trade_cost(dq_call: float, dq_put: float, q: pd.Series) -> float:
            spread_c = max(float(q["c_ask"] - q["c_bid"]), 0.0)
            spread_p = max(float(q["p_ask"] - q["p_bid"]), 0.0)
            spread_cost = (abs(dq_call) * spread_c * 0.5 + abs(dq_put) * spread_p * 0.5) * self.contract_multiplier
            fee_cost = (abs(dq_call) + abs(dq_put)) * self.option_fee_per_contract
        ...
        for i in range(len(df)):
            ...
            if state is not None:
                if str(state["instrument"]) == "risk_reversal":
                    q = self._find_risk_reversal_quote(
                        today,
                        exdate=pd.Timestamp(state["exdate"]),
                        call_strike=float(state["c_strike"]),
                        put_strike=float(state["p_strike"]),
                    )
                else:
                    q = _find_quote(today, exdate=pd.Timestamp(state["exdate"]), strike=float(state["strike"]))
                if q is not None:
            ...
            if abs(pos) > 1e-12 and today is not None and not today.empty:
                if state is not None and str(state["instrument"]) == instrument and float(state["t_days"]) > float(self.roll_threshold_days):
                    if instrument == "risk_reversal":
                        q_same = self._find_risk_reversal_quote(
                            today,
                            exdate=pd.Timestamp(state["exdate"]),
                            call_strike=float(state["c_strike"]),
                            put_strike=float(state["p_strike"]),
                        ...
                        q_same = _find_quote(today, exdate=pd.Timestamp(state["exdate"]), strike=float(state["strike"]))
            ...
            if state is not None and (target_quote is None or not same_contract or str(state["instrument"]) != instrument):
                if str(state["instrument"]) == "risk_reversal":
                    q_close = self._find_risk_reversal_quote(
                        today,
                        exdate=pd.Timestamp(state["exdate"]),
                        call_strike=float(state["c_strike"]),
                        put_strike=float(state["p_strike"]),
                    ...
                    q_close = _find_quote(today, exdate=pd.Timestamp(state["exdate"]), strike=float(state["strike"]))
                if q_close is not None:
                    option_trade_cost += _option_trade_cost(dq_call=abs(float(state["q_call"])), dq_put=abs(float(state["q_put"])), q=q_close)
                state = None
            ...
            if target_quote is not None:
                ...
                if abs(dq_call) > 1e-12 or abs(dq_put) > 1e-12:
                    traded = True
                    option_trade_cost += _option_trade_cost(dq_call=dq_call, dq_put=dq_put, q=target_quote)
            ...
            if state is not None and today is not None:
                if str(state["instrument"]) == "risk_reversal":
                    q_live = self._find_risk_reversal_quote(
                        today,
                        exdate=pd.Timestamp(state["exdate"]),
                        call_strike=float(state["c_strike"]),
                        put_strike=float(state["p_strike"]),
                    ...
                    q_live = _find_quote(today, exdate=pd.Timestamp(state["exdate"]), strike=float(state["strike"]))
